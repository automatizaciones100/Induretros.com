...
interface Props {
  slug: string;
  /** Icono decorativo del header. Default: ShieldCheck por convención legal. */
  icon: LucideIcon;
  /** Título a mostrar si la página no existe en BD todavía. */
  fallbackTitle: string;
  /** Subtítulo bajo el h1. */
  subtitle: string;
  /** Texto cuando no hay contenido en BD. */
  emptyMessage: string;
  /** Configuración del CTA inferior. Si se omite, no se muestra. */
  cta?: {
    title: string;
    description: string;
    primaryLabel: string;
...
/**
 * Renderer compartido para páginas legales (garantía, términos, devoluciones,
 * privacidad, etc.). Recibe el slug de la legal_page en BD y la dibuja con
 * el mismo layout y estilos.
 */
    ...
    <div className="container mx-auto py-12">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-10">
          ...
          <h1 className="section-title">{page?.title || fallbackTitle}</h1>
        ...
        {page ? (
          ...
          <div className="bg-bg-light rounded-xl p-10 text-center">
            <p className="text-gray-mid font-sans">{emptyMessage}</p>
          </div>
        )}
        ...
        {page?.updated_at && (
          <p className="text-xs text-gray-light font-sans text-center mt-4">
            ...
            {new Date(page.updated_at).toLocaleDateString("es-CO", {
              year: "numeric",
              month: "long",
              day: "numeric",
            })}
        ...
        {cta && (
          <div className="mt-12 bg-dark-2 rounded-xl p-8 text-center text-white">
            ...
            <p className="text-gray-300 text-sm mb-6">{cta.description}</p>
