...
function cmplz_tcf_admin_init() {
	$tcf_active = cmplz_iab_is_enabled();

	cmplz_tcf_handle_license_state_transition( $tcf_active );

	if ( $tcf_active ) {
		cmplz_tcf_register_admin_filters();
	}
}

/**
 * Detect transitions in TCF license state (valid↔invalid) and
 * regenerate banner cache when a change is found.
 *
 * @param bool $current Whether TCF is currently active.
 */
function cmplz_tcf_handle_license_state_transition( bool $current ) {
	$previous = get_option( 'cmplz_tcf_license_active', null );

	if ( null === $previous ) {
		// First run: record current state without triggering a resave.
		update_option( 'cmplz_tcf_license_active', $current, false );
		return;
	}
...
/**
 * Register admin-only filters that require an active TCF license.
 */
function cmplz_tcf_register_admin_filters() {
...
function cmplz_tcf_override_auto_updates( $update, $item ) {
	...
	if ( isset( $item->slug ) && strpos( $item->slug, 'complianz-gpdr-premium' ) !== false && ! in_array( 'complianz-gdpr-premium/complianz-gpdr-premium.php', $option, true ) ) {
...
/**
 * Add TCF-specific warning types.
 *
...
function cmplz_tcf_warnings_types( $warnings ) {
	$warnings += array(
		'cmp-file-error' => array(
			...
			'dismissible'        => false,
			'urgent'             => __(
				'The CMP vendorlist files for TCF are not downloaded to, or reachable in the uploads folder yet. If you continue to see this message, contact support to update the files manually.',
				'complianz-gdpr'
			),
		),
		'tcf-22'         => array(
	...
	if ( ! is_array( $auto_updates ) || ! in_array( CMPLZ_PLUGIN, $auto_updates, true ) ) {
		$warnings += array(
...
function cmplz_update_json_files( $attempt = 1 ) {
	...
	$cmplzExistingLanguages = array( 'gl', 'eu', 'bs', 'ar', 'uk', 'bg', 'ca', 'cs', 'da', 'de', 'el', 'es', 'et', 'fi', 'fr', 'hr', 'hu', 'it', 'ja', 'lt', 'lv', 'mt', 'nl', 'no', 'pl', 'pt', 'ro', 'ru', 'sk', 'sl', 'sr', 'sv', 'tr', 'zh' );
...
function cmplz_tcf_set_default( $value, $fieldname, $field ) {
	...
	if ( 'tcf_purposes' === $fieldname ) {
		$value = cmplz_tcf_get( 'purposes', true );
	}

	if ( 'tcf_specialpurposes' === $fieldname ) {
		$value = cmplz_tcf_get( 'specialpurposes', true );
	}

	if ( 'tcf_features' === $fieldname ) {
 ...
 * @return array
 */
function cmplz_tcf_change_settings( $options = array(), $field_id = false, $field_value = false, $prev_value = false, $type = false ) {
	...
	if ( ( $field_value !== $prev_value ) &&
		( 'uses_ad_cookies_personalized' === $field_id ) &&
		( 'yes' === $field_value || 'tcf' === $field_value ) ) {
		// If this setting has just been enabled, re-download the jsons.
		delete_option( 'cmplz_vendorlist_downloaded_once' );
	}

	if ( 'uses_ad_cookies' === $field_id ) {
	...
	if ( 'uses_ad_cookies_personalized' === $field_id && ( 'yes' === $field_value || 'tcf' === $field_value ) ) {
...
function cmplz_tcf_get( string $fieldname, bool $default_on = false ) {
	...
	if ( empty( $items ) ) {
		...
		if ( ! empty( $data ) ) {
			if ( 'specialfeatures' === $fieldname ) {
				$fieldname = 'specialFeatures';
			}
			if ( 'specialpurposes' === $fieldname ) {
				$fieldname = 'specialPurposes';
			}
...
function cmplz_add_tcf_fields( array $fields ): array {
	...
	if ( cmplz_iab_is_enabled() ) {
		...
		foreach ( $hide_fields as $field_id ) {
			...
			$key  = reset( $keys );
			if ( false !== $key ) {
				$fields[ $key ]['react_conditions'] = array(
					'relation' => 'AND',
					array(
		...
		$link_notice_ids  = array( 'title', 'header', 'position', 'colorpalette_button_accept', 'use_custom_cookie_css' );
		$link_notice_keys = array();
			...
			array(
				...
				array(
					...
					'help'             => array(
						...
						'text'  => __(
							'These special purposes should be enabled for best results. These purposes are set based on legitimate interest of the vendor, one of the legal bases of data processing.',
							'complianz-gdpr'
						),
				...
				array(
					...
					'tooltip'          => __(
						"Some vendors will share data outside of the EU/UK. It's disabled by default, but we recommend US based publishers to enable this option.",
						'complianz-gdpr'
					),
					'react_conditions' => array(
						'relation' => 'AND',
				...
				array(
					...
					'tooltip'          => __(
						"Vendors are required to describe their services. Some vendors list services under 'other' and could be considered to negate transparency.",
						'complianz-gdpr'
					),
					'react_conditions' => array(
						'relation' => 'AND',
...
function cmplz_tcf_adjust_cookie_policy_snapshot_html( $html ) {
	...
	foreach ( $p_labels as $key => $label ) {
		if ( ! in_array( $key, $purposes ) ) {
			unset( $p_labels[ $key ] );
		}
	}
	foreach ( $sp_labels as $key => $label ) {
		if ( ! in_array( $key, $special_purposes ) ) {
			unset( $sp_labels[ $key ] );
		}
	}
	...
	foreach ( $sf_labels as $key => $label ) {
		if ( ! in_array( $key, $special_features ) ) {
			unset( $sf_labels[ $key ] );
		}
	}
